...
naiveGarbler :: Int -> Circ -> IO (Acirc2, Circ2, String)
naiveGarbler nseeds c' = do
    (gb, (c, g2Desc)) <- buildGarbler
    return (gb, c, g2Desc)
  where
    buildGarbler = runCircuitT $ do
        let c = toCirc2 c'
        ...
        gs <- forM (zip [0..] (gates c)) $ \(i, (zref, g)) -> do
            ...
            forM (zip (permutations 2 [0,1]) tt) $ \([i,j], [i',j']) -> do
                mx <- g2 j (xwire i)
                my <- g2 i (ywire j)
                foldM1 (zipWithM circXor) [mx, my, zwire (gateEval undefined g [i',j'])]
...
-- XXX: only fan-out one is secure at the moment
-- garbler that does not use permutation bits
indexedGarbler :: Int -> Int -> Circ -> IO (Acirc2, Circ2, String)
indexedGarbler nseeds nindices c' = do
    (gb, (c, g2Desc)) <- buildGarbler
    return (gb, c, g2Desc)
  where
    buildGarbler = runCircuitT $ do
        ...
        gateWires <- lift $ randIO $ forM (gates c) $ \(zref,g) -> do
            ...
            fmap concat $ forM tt $ \[i,j] -> do
                let x = get xs i
                    y = get ys j
                    z = get zs (gateEval undefined g [i,j])
                return (x ++ y ++ z)
        ...
        return (c, g2Desc) -- return the actual circuit garbled along with
                           -- the description of the PRG for evaluation
